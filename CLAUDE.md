...
- **Voltz** (internally still "VexHub" — file/dir names predate the rebrand) — the active app. A React SPA for VEX Robotics students: lessons, a Code Lab (Monaco-based C++/VEXcode editor + AI assistant "Rio"), a 3D CAD builder (Three.js), a team Dashboard, Resources, and a real-time Team Chat (Supabase). **This is what `npm run dev` serves.** Brand: name **Voltz**. Mascot/AI face = **Volt**, a glossy red robot (cyan glowing eyes, lightning-bolt antenna) rendered by `VoltMascot({size})` — a module-level component in VexHub.jsx just above `Nav` (uses `React.useId()` so its gradient ids stay unique across instances). `VoltMascot` (flat inline-SVG) is used at small/tiny sizes: the `Nav` mark and the assistant message avatars. A **real 3D Volt** (`VoltModel` + `Volt3D`, react-three-fiber — glossy white shell, glowing red visor, idle rotate/bob) is used on the `FloatingChat` launcher button and the chat header avatar (dark backdrop so the white shell pops). Note: do NOT wrap `Volt3D` in a `scale:0` entrance (e.g. `popIn`) — r3f then measures the canvas at 0px and it stays tiny; that's why the launcher has no pop-in. The **AI assistant is named "Voltz"** (renamed from the old "Rio" — intro lines, system prompts in CodeLab + FloatingChat, aria-labels, header all say Voltz). `VoltzMark` (red squircle + bolt) and `VoltzBolt` (bolt glyph) still exist as the simpler flat logo; the browser tab/bookmark favicon + apple-touch-icon is `public/volt.png` (the Volt robot — set in index.html; `favicon.svg` bolt still exists but is no longer referenced); tab `<title>` is "Voltz — Robotics, charged". The nav wordmark uses the `.brand-wordmark` class (Sora 800 display font, loaded via a Google Fonts link in index.html; defined in index.css). "VEX" appears only as descriptive/nominative copy (e.g. hero eyebrow "Learn VEX Robotics Smarter"), never as the product/assistant name.
...
└── supabase/
    ...
    │   ├── 20260810_analytics.sql           ← superseded by 20260825_site_events.sql — kept for history, do not apply standalone on a fresh project
    │   ├── 20260824_feedback.sql            ← superseded by 20260825_site_events.sql — kept for history, do not apply standalone on a fresh project
    │   └── 20260825_site_events.sql         ← `site_events` table — ONE table for both visits and feedback, split by `kind` ('visit'|'feedback'); columns: visitor_id/user_id/path (visits), type/message/rating (feedback). anon INSERT only, no SELECT. `get_site_stats()` (kind='visit') and `get_feedback(lim)` (kind='feedback') SECURITY DEFINER RPCs — same contracts as before. Migrates + drops the old site_visits/feedback tables if present. Powers the owner Site-stats panel + FeedbackWidget/OwnerStats feedback list
    └── functions/
...
| 229–386 | `AuthProvider` / `useAuth` | Supabase Auth context — `user`, `signIn`/`signUp` (email+password), `signInWithGoogle` (OAuth, redirects to Google consent), `signOut`, `resetPasswordForEmail` (mails a one-time reset link), `passwordRecovery`/`updatePasswordAndClearRecovery` (set true by the `PASSWORD_RECOVERY` auth event when someone clicks that link; `ResetPasswordModal` gates them into setting a new password, then clears it). On mount it also handles **email-link landings**: a `?token_hash=&type=` URL is verified with `verifyOtp()` (works in ANY browser — the PKCE `?code=` exchange needs the originating browser's code_verifier, so cross-device confirmations landed signed out), `type=recovery` sets `passwordRecovery` by hand since verifyOtp emits SIGNED_IN, link errors (`error_code=otp_expired` etc., query OR hash) raise a friendly toast, and the auth params are stripped via replaceState. Guarded by a module-level `emailLinkHandled` so StrictMode's double-invoked effect can't burn the single-use token. Requires the Supabase email templates to use `{{ .TokenHash }}` (see DEPLOY.md). Drives dashboard cloud sync + the sign-in nudge |
...
| — | `FeedbackWidget` | **Feedback (2026-08).** Floating bottom-left "Feedback" button (mounted in app shell, hidden on codelab/community like FloatingChat) → modal with just a message field (no type pills — user request) → `submitFeedback()` inserts a `site_events` row with `kind:"feedback"`, `type:"other"` (auto user_id when signed in). Owner reviews recent feedback inside the PIN-gated `OwnerStats` panel (via `fetchFeedback()`/`get_feedback` RPC); full rows incl. user_id readable in the Supabase Table editor. Needs `20260825_site_events.sql` applied |
| — | `GoogleButton`, `SignInPrompt`, `OwnerStats` | **Auth/analytics (2026-08).** `GoogleButton` = reusable "Continue with Google" (Supabase OAuth) — in AuthModal + SignInPrompt. `SignInPrompt` = **hard sign-in gate** (mounted in app shell) — covers the app `FORCE_SIGNIN_AFTER_MS` (30s) after a signed-out visitor lands and is **not dismissible** (no "Maybe later", no backdrop close, no cooldown); signing in is the only way past. Hidden for signed-in users. (Was previously a soft, dismissible, engagement-triggered nudge; `shouldPromptSignIn`/`getLastPromptAt`/`markPromptShown` remain exported + tested in lib/analytics.js but are no longer imported by VexHub.) `OwnerStats` = PIN-gated (reuses `checkPin`) Site-stats panel in the Dashboard footer showing visits/unique-visitors/signed-in/today via the `get_site_stats()` RPC (`fetchSiteStats`); shows an "apply the migration" hint until `20260825_site_events.sql` is applied. `recordVisit()` (module-level, near getSB) inserts one `site_events` row (`kind:"visit"`) per browser session, called only after `authLoading` resolves so `user_id` is accurate |
...
- **In-call shared whiteboard (Meet/Teams style)**: the call toolbar's Whiteboard button embeds an **Excalidraw** board *inside* the call (iframe beside the LiveKit video, which stays mounted), not a new tab. The room is derived deterministically from `serverId` (SHA-256 → `#room=<20-hex>,<128-bit base64url key>`) so everyone shares one board; `deriveWhiteboardUrl()` builds it. Open/close is synced to all call participants via an ephemeral broadcast channel `wb:${serverId}_${channel}` (`openWhiteboard`/`closeWhiteboard`/`wbChanRef`, no DB) so one person opening it opens it for everyone. Reset on leaving the call. Excalidraw sends no X-Frame-Options so it embeds cleanly.
...
- **Built-in profanity/slur filter (always-on)**: independent of the owner's custom rules and applied to **every** server incl. the public Community. `containsBannedWord()` + `DEFAULT_BLOCKLIST` + `normalizeForFilter()` in `lib/chatGuards.js` block strong profanity, explicit sexual terms, and hate slurs (the categories mainstream platforms disallow). Normalises first (lowercase, leet-map `0134578@$|!`→letters, non-letters→space, collapse 3+ repeats) then matches each term with a leading `\b` + optional suffix + trailing `\b` (avoids the Scunthorpe problem — "class"/"assess"/"cockpit" pass), plus a tiny de-spaced pass for a few severe slurs (catches "n i g g e r"). Enforced in `send()` and `sendMedia()` (caption) before the owner rules; blocks with a generic message. **Client-side UX guard, not a hard boundary** (see SECURITY.md). Tested in `chatGuards.test.js`.
- **Automod (text matching)** — owner-configured, on top of the built-in filter:
...
- **Server roles (team servers, 2026-08)**: three tiers. **Owner** = the creator (`adminUser`, from `server_config.share_data.admin`) — immune, and the only one who can grant/revoke roles. **Admin** = owner OR any name in `adminList` (moderator: manage channels, edit notebook) — cannot manage roles and cannot touch the owner. **Member** = everyone else. `isOwner`/`isAdmin`/`roleOf()` are derived (not state) after `myName`. The granted-admin list lives in a `share_type:"roles_config"` sys record (`share_data.admins:[]`), latest-wins, realtime-synced into `adminList`; `saveRoles()`/`grantAdmin()`/`revokeAdmin()` are owner-gated and write it (optimistic + realtime echo). The members panel shows Owner/Admin/Member groups with owner-only "+ Admin"/"− Admin" hover buttons (never on the owner row). **Client-side enforcement only** (like automod/bans — see SECURITY.md).
...
**Analytics + feedback setup:** apply `supabase/migrations/20260825_site_events.sql` (creates the combined `site_events` table + `get_site_stats()`/`get_feedback()`; migrates and drops any existing `site_visits`/`feedback` tables from the older migrations). Until applied, visit/feedback inserts 404 harmlessly and the Site-stats panel shows a "not set up yet" hint.
...
- **Coverage today** (`src/lib/*.test.{js,jsx}`):
  - `chatGuards.test.js` — automod matching (regex + substring fallback + bad input), built-in profanity/slur filter (`containsBannedWord`/`normalizeForFilter`: leet + repeats + suffixes + letter-spaced slurs + Scunthorpe-safe negatives), IP-ban fail-open, rate-limit windows, daily-upload accounting, file-size cap, server join/create validation, formatting, constant values.
