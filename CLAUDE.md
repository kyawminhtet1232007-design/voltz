...
└── supabase/
    ...
    │   └── 20260824_feedback.sql            ← `feedback` table (type/message/rating/user_id/path) — anon INSERT only, no SELECT; `get_feedback(lim)` SECURITY DEFINER RPC returns recent feedback content (no user_id/PII). Powers the FeedbackWidget + OwnerStats feedback list
    └── functions/
